...
- `reference source registry` 已落地：`Tushare hk_basic` 已回填 `listing_date`，`HKEX REIT` curated seed 已补入当前 REIT override，`OpenD` 已接成 current snapshot secondary source；`港股通` 仍保留 time-bounded curated seed 入口
